...
{
    model = new Model(objFilename, tx, ty, tz, alpha, beta, gamma, scale);
    ...
    this->tclcHistograms = new TCLCHistograms(model, 16, 40, 10.0f);
...
Object3D::Object3D(Model* model_, float qualityThreshold, vector<float>& templateDistances)
{
    model = model_;
...
Object3D::Object3D(Model* model_)
{
    model = model_;
}
...
Object3D::~Object3D()
{
    if (tclcHistograms) { delete tclcHistograms; }
...
void Object3D::generateTemplates()
{
    ...
    int baseGammaSteps = 360 / 90;  // = 4
    int baseTemplateCount = baseIcosahedron.size() * baseGammaSteps * numDistances;

    baseTemplates.resize(baseTemplateCount);
...
#pragma omp parallel for
    for(int i = 0; i < baseIcosahedron.size(); i++)
    {
        ...
        for (int g = 0; g < baseGammaSteps; g++) {
            int gamma = g * 90;
            for (int d = 0; d < numDistances; d++) {
                int index = i * baseGammaSteps * numDistances + g * numDistances + d;
                baseTemplates[index] = new TemplateView(this, alpha, beta, gamma, templateDistances[d], numLevels, true);
            }
        }
    }
    
    int gamma2Precision = 30;
    int neighborGammaSteps = 360 / gamma2Precision;
    int neighborTemplateCount = subdivIcosahedron.size() * neighborGammaSteps * numDistances;

    neighboringTemplates.resize(neighborTemplateCount);
...
#pragma omp parallel for
    for(int i = 0; i < subdivIcosahedron.size(); i++)
    {
        ...
        for (int g = 0; g < neighborGammaSteps; g++) {
            int gamma = g * gamma2Precision;
            for (int d = 0; d < numDistances; d++) {
                int index = i * neighborGammaSteps * numDistances + g * numDistances + d;
                neighboringTemplates[index] = new TemplateView(this, alpha, beta, gamma, templateDistances[d], numLevels, true);
            }
        }
    }
