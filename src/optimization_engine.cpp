...
void OptimizationEngine::runIteration(vector<Object3D*>& objects, const vector<Mat>& imagePyramid, int level)
{
    ...
    {
        if(objects[o]->getModel()->isInitialized())
        {
    ...
    for(int o = 0; o < objects.size(); o++)
    {
        if(objects[o]->getModel()->isInitialized())
        {
            ...
            renderingEngine->renderSilhouette(objects[o]->getModel(), GL_FILL, true);
            ...
            int m_id = (numInitialized <= 1) ? -1 : objects[o]->getModel()->getModelID();
...
Rect OptimizationEngine::compute2DROI(Object3D* object, const cv::Size& maxSize, int offset)
{
    ...
    renderingEngine->projectBoundingBox(object->getModel(), projections, boundingRect);
