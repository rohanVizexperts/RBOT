...
public:
    ...
    Model(const std::string modelFilename, float scale);
    ...
    void InitializeModelPose(cv::Matx33f &K, int width, int height);
    ...
    /*
    * Returns the Position of the model from the camera (in mm)
    */
    cv::Vec3f getPosition();

    /*
    * Returns the rotation of the model in X, Y, Z from the camera (in Degree)
    */
    ...
    /**
     *  Gets the initial model pose to a given 6DOF rigid body
     *  transformation in form of a 4x4 float matrix
     *  T_cm = [r11 r12 r13 tx]
     *         [r21 r22 r23 ty]
     *         [r31 r32 r33 tz]
